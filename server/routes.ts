...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/sample-data", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as Express.User).id;
      ...
      const calendarEvents = [
        ...
        },
        {
          ...
          start_time: new Date(new Date(tomorrow).setHours(14, 0, 0, 0)),
          end_time: new Date(new Date(tomorrow).setHours(15, 30, 0, 0)),
          location: "Zoom",
          created_at: new Date()
        },
        {
          ...
          description: "Discuss marketing strategy for new product launch",
          start_time: new Date(new Date(nextWeek).setHours(11, 0, 0, 0)),
          end_time: new Date(new Date(nextWeek).setHours(12, 30, 0, 0)),
      ...
      const tasks = [
        {
          ...
          description: "Create 10 slides for the client meeting tomorrow",
          due_date: tomorrow,
          completed: false,
          created_at: new Date()
        },
        {
          ...
          description: "Analyze Q2 expenses and prepare report",
          due_date: new Date(new Date().setDate(today.getDate() + 3)),
          completed: false,
          created_at: new Date()
        },
        {
          ...
          description: "Send recap emails to all clients from yesterday's meetings",
          due_date: today,
          completed: true,
          completed_at: new Date(),
          created_at: new Date(new Date().setDate(today.getDate() - 1))
        },
        {
          user_id: userId,
          title: "Update website content",
          description: "Add new product information to the company website",
          due_date: new Date(new Date().setDate(today.getDate() + 5)),
      ...
      for (const event of calendarEvents) {
        const addedEvent = await storage.createCalendarEvent(event);
        addedEvents.push(addedEvent);
      }
      
      const addedTasks = [];
      for (const task of tasks) {
        const addedTask = await storage.createTask(task);
